...
- Added deterministic, bounded typed-spreadsheet explanation queries for current-cell semantics and latest-edit evidence. Typed result/kind/change variants replace stringly query states, context schema v2 carries structured before/after evidence, and the Rabbita demo retains one coherent `ExplanationFrame` behind a default-closed `explain <cell>` inspector.
