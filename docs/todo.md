...
- [ ] **Disposed-cell-on-same-signal anomaly** — 100 disposed reactives on a signal cost 27 µs per `Signal::set`, vs 45 ns when the disposed cells are on a different signal. `push.node_count == 0` after the last dispose, so `push_propagate_from` is skipped — the cost must come from another path. **First step (N=100, not 1):** existing one-reactive dispose assertions at `cells/push_reactive_wbtest.mbt:303-314` confirm `sig.subscribers.length() == 0` after a single dispose. The anomaly only shows up at N=100, so write a new whitebox bench that disposes 100 reactives on one signal and inspects (i) `sig.subscribers.length()`, (ii) `rt.push.node_count`, (iii) HashSet tombstone/capacity state on the subscriber set (does `remove_subscriber` shrink the bucket array, or leave 100 tombstones that `Signal::set` still walks?). Two outcomes: (a) subscribers empty + HashSet shape clean → trace the other path (cell_index walk? cell_ops dispatch?); (b) HashSet has tombstones or stale capacity → `remove_subscriber` lifecycle issue + sizing strategy. Potentially much larger impact than per-reactive optimizations for cell-churn workloads (editor sessions). Surfaced by the push-engine cost-decomposition investigation 2026-05-16.
