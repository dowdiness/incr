...
cd "$ROOT"
SPIKE=examples/spikes/incr_next_formula_exports
NEG="$SPIKE/negative"
TMP="$NEG/probe.mbt"
COPY_TMP=$(mktemp -d)
trap 'rm -f "$TMP"; rm -rf "$COPY_TMP"' EXIT

printf '%s\n' '== Incr Next Formula export evidence =='
...
mbti_files=(
  ...
  derived_provider/pkg.generated.mbti
  downstream/pkg.generated.mbti
  negative/pkg.generated.mbti
)
moon info "$COPY_TMP/program" "$COPY_TMP/feature_provider" \
  "$COPY_TMP/derived_provider" "$COPY_TMP/downstream" "$COPY_TMP/negative"
...
moon check --target native \
  "$SPIKE/program" "$SPIKE/feature_provider" "$SPIKE/derived_provider" \
  "$SPIKE/downstream" "$SPIKE/negative"
moon check --target wasm-gc \
  "$SPIKE/program" "$SPIKE/feature_provider" "$SPIKE/derived_provider" \
  "$SPIKE/downstream" "$SPIKE/negative"
moon test --target native "$SPIKE/downstream"
moon test --target wasm-gc "$SPIKE/downstream"

printf '%s\n' '-- native/wasm exact output --'
native_output=$(moon run "$SPIKE/downstream" --target native)
wasm_output=$(moon run "$SPIKE/downstream" --target wasm-gc)
printf '%s\n' "$native_output"
printf '%s\n' "$wasm_output"
...
grep -Fxq 'formula_exports workloads=true' <<<"$native_output"

printf '%s\n' '-- unchanged #461-#468 executable evidence --'
...
moon run examples/spikes/incr_next_cross_program_ports/feature_consumer --target native
...
printf '%s\n' '-- privacy, package boundaries, docs, and prior-tree guards --'
if rg -n '@kernel|incremental_provider|View\[|Query\[' \
  "$SPIKE/feature_provider/pkg.generated.mbti" \
  "$SPIKE/derived_provider/pkg.generated.mbti" \
  "$SPIKE/downstream/pkg.generated.mbti"; then
  printf '%s\n' 'raw kernel View/Query type leaked through a feature interface' >&2
  exit 1
fi
if rg -n 'kernel|incremental_provider' "$SPIKE/derived_provider/moon.pkg" \
  "$SPIKE/downstream/moon.pkg"; then
  printf '%s\n' 'derived provider or downstream imports the kernel' >&2
  exit 1
fi
if [[ "$(grep -c 'incr_next_formula_exports/feature_provider' "$SPIKE/derived_provider/moon.pkg")" -ne 1 ]] ||
  [[ "$(grep -c 'incr_next_formula_exports/program' "$SPIKE/derived_provider/moon.pkg")" -ne 1 ]]; then
  printf '%s\n' 'derived provider import boundary changed' >&2
  exit 1
fi
...
npx slopless "$SPIKE/README.md" >/tmp/incr-next-formula-exports-slopless.json
python3 - <<'PY'
import json
with open('/tmp/incr-next-formula-exports-slopless.json') as handle:
...
git diff --exit-code e6b1e0e -- examples/spikes/incr_next_cross_program_ports
git diff --cached --exit-code e6b1e0e -- examples/spikes/incr_next_cross_program_ports
if git status --short -- examples/spikes/incr_next_fresh_evaluator \
  ...
  examples/spikes/incr_next_cross_program_ports | grep -q .; then
...
forbidden_paths=$(grep -Ev '^(moon\.work|examples/spikes/README\.md|examples/spikes/incr_next_formula_exports(/.*)?)$' <<<"$changed_paths" || true)
if [[ -n "$forbidden_paths" ]]; then
  printf '%s\n' 'change outside the Formula export spike allowlist:' >&2
  printf '%s\n' "$forbidden_paths" >&2
  exit 1
fi

printf '%s\n' 'harness state: fmt=PASS info=PASS checks=PASS tests=PASS native=PASS wasm=PASS prior=#461-#468-PASS negatives=PASS privacy=PASS slopless=PASS'
grep -Fq 'The evidence verdict is **Pass with constraints**.' "$SPIKE/README.md"
printf '%s\n' 'constrained verdict: PASS - owner-scoped Formula exports preserve explicit multi-hop edges and error channels.'
