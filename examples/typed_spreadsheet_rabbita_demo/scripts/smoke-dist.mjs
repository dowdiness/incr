...
try {
  ...
  assert(html.includes('<title>Typed sheet editor</title>'), 'Built HTML is missing the typed sheet title');
  assert(html.includes('id="app"'), 'Built HTML is missing the app mount node');
  assert(
    html.includes('Loading typed sheet editor'),
    'Built HTML is missing the typed sheet loading shell',
  );
  ...
  assert(
    scripts.some(script => script.includes('50 by 50 typed spreadsheet')),
    'Built JS bundle is missing the typed spreadsheet grid view',
  );
  assert(
    scripts.some(script => script.includes('Ready. Edit the formula bar')),
    'Built JS bundle is missing the worksheet editor model',
  );
  assert(
    stylesheets.some(stylesheet => stylesheet.includes('.sheet-workspace')),
    'Built CSS bundle is missing the sheet workspace styles',
  );
