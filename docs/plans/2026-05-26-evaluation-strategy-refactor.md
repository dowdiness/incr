...
| Backdating and `changed_at` propagation | `incr/cells/backdating_test.mbt`; `incr/cells/introspection_test.mbt` (`memo: changed_at and verified_at track revisions`); `incr/cells/target_facade_wbtest.mbt` (`Derived::changed_at forwards to inner Memo`); `incr/cells/reachable_derived_wbtest.mbt` (`hybrid memo: backdating — unchanged value does not bump changed_at`) | **Covered.** Pull backdating, explicit `changed_at` movement, facade forwarding, and custom equality behavior are pinned. |
