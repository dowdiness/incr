...
MOONBIT_FFI_EXPORT
RcProbeToken *incr_next_fresh_token_new(int32_t kind) {
...
MOONBIT_FFI_EXPORT
void incr_next_fresh_token_touch(RcProbeToken *token) {
  (void)token->kind;
}

MOONBIT_FFI_EXPORT
int32_t incr_next_fresh_live(int32_t kind) {
  return kind >= 0 && kind < RC_PROBE_KIND_COUNT ? live_counts[kind] : -1;
}

MOONBIT_FFI_EXPORT
int32_t incr_next_fresh_finalized(int32_t kind) {
