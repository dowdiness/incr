...
- [ ] **Static/applicative `Derived` fast-path.** Inspired by the "Build Systems à la Carte" applicative-vs-monadic task distinction. Decision refreshed 2026-05-31 after re-running the tracked probe benches on current main: proceed privately, but keep public API design deferred. Package-private generalized path now returns normal `Derived` handles backed by `MemoData`/`PullMemo`, registers fixed dependencies once, and skips tracking-stack allocation plus dependency-list diffing on recompute. Fresh integrated scalar benches show 1.42×/1.55×/1.85× speedups on wasm-gc map1/2/3 and 1.58×/1.76×/1.89× on JS; lower-bound dynamic-vs-manual probes still show large headroom ([probe](performance/2026-05-27-static-derived-fast-path-probe.md)). UI-shape variants show layered fanout at noise/small-win level, but the tree-shaped graph still wins (~1.11× wasm-gc, ~1.24× JS). Private hardening covers static-to-static cleanup/GC, duplicate dependency normalization, failure cleanup, fixed-dep cycle reporting, explicit accumulator rejection, declared-dependency-only source reads, normal `Watch` rooting, and honest `ReadError::Disposed` for direct disposed reads. Public-surface options are recorded in [2026-05-28 Static Derived Public-Surface Options](design/specs/2026-05-28-static-derived-public-options.md). Next step: decide whether/when a public surface is warranted; do not implement public `Derived::map`, `map2`, `map3`, `Scope::derived_static`, or a raw static installer from the options note alone.
