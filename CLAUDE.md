...
- Cross-module workspace contracts are enforced by `scripts/check-workspace-boundaries.sh` (#343, CI job "Check architecture boundaries"): non-library workspace members (`docs/`, `examples/*`) import only the `dowdiness/incr` root facade (never `/cells`, `/types`, or deeper), and their `dowdiness/incr@X` pins must equal the version in `incr/moon.mod`. The checker's own parsing is regression-guarded by `scripts/check-workspace-boundaries-selftest.sh` (fixture-based known-positive controls, first step of the CI job).
- **Version bumps must be atomic** with member pins (the pin invariant above fails CI otherwise): use `bash scripts/bump-version.sh <semver>`, which bumps `incr/moon.mod` plus every member pin and re-runs the boundary check. `moon publish` follows immediately after the bump merges.
