...
- [ ] **Step 1.1: Create `cells/internal/shared/moon.pkg`**
...
- [ ] **Step 1.2: Create `cells/internal/shared/cell_meta.mbt`**
...
- [ ] **Step 1.3: Create `cells/internal/shared/cell_ref.mbt`**
...
- [ ] **Step 1.4: Create `cells/internal/shared/cell_ops.mbt`**
...
- [ ] **Step 1.5: Strip `cells/cell_ops.mbt` to only `CellLifecycle`**
...
- [ ] **Step 1.6: Update `cells/cell.mbt` to re-export `CellRef`**
...
- [ ] **Step 1.7: Delete `cells/cell_ref.mbt`**

```bash
rm cells/cell_ref.mbt
```

- [ ] **Step 1.8: Update `cells/moon.pkg` to import `@shared`**
...
- [ ] **Step 1.11: Regenerate `.mbti` and diff**
...
- [ ] **Step 2.1: Create `cells/internal/pull/moon.pkg`**
...
- [ ] **Step 2.2: Move `PullSignalData` source from `cells/pull_signal.mbt` into `cells/internal/pull/pull_signal.mbt`**
...
- [ ] **Step 2.3: Create `cells/pull_lifecycle.mbt` holding the `PullSignalData` lifecycle impl**
...
- [ ] **Step 2.8: Run `moon test`**

Expected: `passed: 506, failed: 0`.

- [ ] **Step 2.9: Regenerate `.mbti` and diff**
...
- [ ] **Step 3.1: Create `cells/internal/push/moon.pkg`**
...
- [ ] **Step 3.2: Create `cells/internal/push/push_reactive_data.mbt`**
...
- [ ] **Step 3.5: Trim `cells/push_reactive.mbt` to the handle half**
...
- [ ] **Step 3.6: Trim `cells/push_effect.mbt` to the handle half**
...
- [ ] **Step 3.9: Update `cells/moon.pkg`**

Append `"dowdiness/incr/cells/internal/push" @push,` to the first import block.

- [ ] **Step 3.10: Run `moon check`, `moon test`, `moon info && moon fmt`, diff `.mbti`**
...
- [ ] **Step 4.1: Create `cells/internal/datalog/moon.pkg`**
...
- [ ] **Step 4.2: Create `cells/internal/datalog/relation_data.mbt`**
...
- [ ] **Step 4.3: Create `cells/internal/datalog/functional_relation_data.mbt`**

Same pattern for `FunctionalRelationData` (source: `cells/datalog_functional_relation.mbt` lines ~1-28).

- [ ] **Step 4.4: Create `cells/internal/datalog/rule_data.mbt`**
...
- [ ] **Step 4.5: Create `cells/datalog_lifecycle.mbt`**
...
- [ ] **Step 4.6: Trim `cells/datalog_relation.mbt` to the handle half**
...
- [ ] **Step 4.7: Trim `cells/datalog_functional_relation.mbt`**

Same pattern: remove lines ~1-28 plus the lifecycle impl, add `using @datalog { type FunctionalRelationData }`.

- [ ] **Step 4.8: Trim `cells/datalog_rule.mbt` to `Runtime::new_rule` + helpers**
...
- [ ] **Step 4.9: Update `cells/datalog_fixpoint.mbt`**
...
- [ ] **Step 4.10: Update other consumer imports**
...
- [ ] **Step 4.11: Update `cells/moon.pkg`**

Append `"dowdiness/incr/cells/internal/datalog" @datalog,` to the first import block.

- [ ] **Step 4.12: Run `moon check`, `moon test`, `moon info && moon fmt`, diff `.mbti`**
...
- [ ] **Step 5.1: Create `scripts/check-engine-isolation.sh`**
...
This is the authoritative version. The spec's Layer 1 section contains the same content; if they ever diverge, the spec wins.
...
- [ ] **Step 5.3: Run the script against the current tree**
...
- [ ] **Step 5.4: Smoke-test the failure mode**
...
- [ ] **Step 5.5: Add the script to the standard per-commit verification**
...
- [ ] **Step 5.6: Run full verification**
...
- [ ] **Step 6.1: Update `CLAUDE.md` package map**
...
- [ ] **Step 6.3: Update `docs/design.md`**
...
- [ ] **Step 6.4: Update `docs/README.md`**
...
- [ ] **Step 6.5: Run `bash check-docs.sh`** (if it exists)
...
- [ ] **Step 6.6: Final full verification**
...
- [ ] **All commits present, branch pushable.**
