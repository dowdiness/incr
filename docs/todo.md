...
- [ ] **Static/applicative `Derived` fast-path probe.** Inspired by the "Build Systems à la Carte" applicative-vs-monadic task distinction. Gate accepted 2026-05-27: lower-bound and generalized private prototype benches show the fixed-dependency path recovers ~46–61% of the dynamic/manual gap on wasm-gc and ~52–59% on JS, with a 1.6–2.1× speedup over dynamic `Derived` on stale map1/2/3 recomputes ([probe](performance/2026-05-27-static-derived-fast-path-probe.md)). UI-shape variants show layered fanout at noise level but a material tree-shaped win (~1.35× wasm-gc, ~1.45× JS). The current package-private probe reuses the existing pull backend (`MemoData`/`PullMemo` internal storage), registers fixed dependencies once, and skips tracking-stack allocation plus dependency-list diffing on recompute. Private hardening now covers static-to-static cleanup/GC, duplicate dependency normalization, failure cleanup, fixed-dep cycle reporting, explicit accumulator rejection, and a declared-dependency-only static source read helper. Public-surface options are recorded in [2026-05-28 Static Derived Public-Surface Options](design/specs/2026-05-28-static-derived-public-options.md). Next step: choose whether to keep the path private, design the smallest direct fixed-source API, or target scope-owned attachment pipelines first; do not implement public `Derived::map`, `map2`, `map3`, or `Scope::derived_static` from the options note alone.
