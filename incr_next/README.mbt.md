...
K1.1 provides opaque `View` recipes, tracked `QueryContext` reads, canonical `Source` views, no-memo `Query[K, V]`, Store-owned revisions, transaction-only publication, same-Store cross-Region reads, and explicit Region lifetime.
