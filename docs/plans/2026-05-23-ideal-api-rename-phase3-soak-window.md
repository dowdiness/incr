...
`HybridMemo::read_permissive`** ([cells/reachable_derived.mbt:179](../../cells/reachable_derived.mbt)).
`read_permissive` calls `read_result_inner` directly, skipping the trailing
event-broadcast drain that `read_result` performs ([cells/reachable_derived.mbt:109-122](../../cells/reachable_derived.mbt)).
...
- Bare `#deprecated("msg")` compiles on public methods today
  ([cells/watch.mbt:223](../../cells/watch.mbt)).
...
| `cells/derived_map.mbt` doc comment | `by_id.get(1)` example | 1 docstring |
| `tests/tracked_struct_test.mbt` | `.get_result()` | 1 |
| `types/cycle_error.mbt` doc | `memo.get_result()` example | 1 docstring |
| `cells/derived_test.mbt` / `derived_map_test.mbt` / `derived_raise_wbtest.mbt` / `subscriber_*.mbt` etc. | various | ~30+ |
...
| `cells/reachable_derived_wbtest.mbt` | `.get()` on HybridMemo | 1+ (Codex gap) |
...
`cells/introspection.mbt:59`, `cells/derived_map.mbt:18`, `types/cycle_error.mbt:22`).
...
`Memo::read_result` ([cells/derived.mbt:217-232](../../cells/derived.mbt)); the
public `Memo::get_result` ([cells/derived.mbt:200](../../cells/derived.mbt)) is a
...
in the public `MemoMap::get` body ([cells/derived_map.mbt:57-63](../../cells/derived_map.mbt)),
...
| 5 | Three additional caller files missed: `tests/integration_test.mbt`, `cells/push_reachable_wbtest.mbt`, `cells/reachable_derived_wbtest.mbt` | Added to inventory |
