...
- Made `Program` mount ownership global across renderer instances, preventing
  ...
  final global mount release also discard queued messages and after-flush
  callbacks.
...
- Restored controlled form properties (`value`, `checked`, `disabled`, and
  ...
  live value on `<input>`, `<select>`, and `<textarea>` elements. Select values are
...
- Added deterministic, bounded typed-spreadsheet explanation queries for current-cell semantics and latest-edit evidence. The Rabbita demo now exposes them through one default-closed `explain <cell>` inspector with trace/evidence details behind disclosure.
