...
- [ ] **Disposed-cell-on-same-signal anomaly** — 100 disposed reactives on a signal cost 27 µs per `Signal::set`, vs 45 ns when the disposed cells are on a different signal. `push.node_count == 0` after the last dispose, so `push_propagate_from` is skipped — the cost must come from another path. First step: whitebox-test whether `sig.subscribers.size()` is actually 0 after disposing all reactives. Two outcomes: (a) subscribers empty → trace the other path; (b) subscribers still has entries → `remove_subscriber` lifecycle bug. Potentially much larger impact than per-reactive optimizations for cell-churn workloads (editor sessions). Surfaced by the push-engine cost-decomposition investigation 2026-05-16.
