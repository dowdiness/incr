...
- **`fn new(..)` inside `struct`.** MoonBit v0.9.2 deprecated declaring constructors inside the struct body in favour of a separated `fn Type::Type(...)` toplevel. All 10 library types (`Runtime`, `Signal`, `Memo`, `HybridMemo`, `MemoMap`, `TrackedCell`, `Relation`, `FunctionalRelation`, `ActiveQuery`, `BatchFrame`) now use the new form. Four test-only types (`Point`, `Versioned` in `cells/custom_eq_test.mbt`; `TestDb`, `CalcPipeline` in `tests/traits_test.mbt`) still use the old form — migrate when touching those tests.
