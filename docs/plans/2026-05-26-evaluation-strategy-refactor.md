...
- Kernel packages cannot import `incr/cells/` types that mention `Runtime`. Any
  observer trait that takes `Runtime` lives in `incr/cells/`; kernel code returns or
...
| --- | --- | --- |
| Pull durability fast path with and without accumulator reads | `incr/cells/durability_wbtest.mbt` (`durability: high-only memo skips verify when low input changes`, `durability: shortcut boundary — High memo stays skipped when durability_last_changed[High] equals verified_at`); `incr/cells/accumulator_wbtest.mbt` (`durability shortcut: bypassed when memo has accumulator_reads`, `synthetic dep check: reader invalidates when producer's push_revised_at bumps`, `verify: non-root frame checks accumulator_reads`) | **Covered.** Normal durability shortcuts and the accumulator-read exception are both pinned. |
| Cycle detection through ordinary dependencies | `incr/cells/verify_path_test.mbt`; `incr/cells/cycle_path_test.mbt`; `incr/cells/cycle_test.mbt`; `incr/cells/reachable_derived_wbtest.mbt` (`panic hybrid cycle: mixed Memo → HybridMemo → Memo cycle aborts`) | **Covered.** Ordinary pull cycles, self-cycles, path formatting, re-verification cycles, and mixed Memo/Hybrid cycles are pinned. |
| Cycle detection through synthetic accumulator dependencies | `incr/cells/accumulator_wbtest.mbt` (`verify: synthetic accumulator dep surfaces target cycle and cleans up`, plus synthetic dependency recording/invalidation tests) | **Covered.** Synthetic accumulator verification reaches a cyclic target, returns a `CycleError`, and clears in-progress/tracking state. |
| Compute failure cleanup of `in_progress` and tracking stack | `incr/cells/accumulator_wbtest.mbt` (`verify: raise-path from compute does not leak in_progress on ancestors`); `incr/cells/accumulator_commit_hook_wbtest.mbt` (`MemoCommitPhase: forward dispatch order on abort`, `MemoCommitPhase: nested recompute aborts clean up both entries`); `incr/cells/event_broadcast_hook_wbtest.mbt` (`memo_event: hook order on abort removes active entries before later hooks`); `incr/cells/verify_wbtest.mbt` leak simulations | **Covered.** Raised compute cleanup directly asserts empty tracking stack and false `in_progress` flags, with hook cleanup pinned separately. |
| Dynamic dependency replacement | `incr/tests/integration_test.mbt` (`integration: dynamic dependencies`); `incr/cells/derived_dep_diff_wbtest.mbt` (`dep diff: dynamic deps update dep list and durability`); `incr/cells/subscriber_link_wbtest.mbt` (`subscriber: dynamic dep changes update subscriber links`); `incr/tests/subscriber_test.mbt` (`subscriber: dynamic deps update dependents via public API`) | **Covered.** Value behavior, dependency lists, durability recomputation, and subscriber links are pinned. |
| Backdating and `changed_at` propagation | `incr/cells/backdating_test.mbt`; `incr/cells/introspection_test.mbt` (`memo: changed_at and verified_at track revisions`); `incr/cells/target_facade_wbtest.mbt` (`Derived::changed_at forwards to inner Memo`); `incr/cells/reachable_derived_wbtest.mbt` (`hybrid memo: backdating — unchanged value does not bump changed_at`); `incr/tests/backdate_eq_test.mbt` | **Covered.** Pull backdating, explicit `changed_at` movement, facade forwarding, and custom equality behavior are pinned. |
| Push diamond glitch prevention | `incr/cells/eager_derived_wbtest.mbt` (`push propagation: glitch prevention in diamond`); related pull diamond smoke in `incr/tests/integration_test.mbt` (`integration: diamond dependency`, `integration: diamond with backdating`) | **Covered.** Push diamond consistency is directly pinned. |
| Push stale queue-entry skip behavior | `incr/cells/eager_derived_wbtest.mbt` (`push propagation: skip stale queue entry after mid-wave disposal`) | **Covered.** Stale queued push node disposal is directly pinned. |
| Push/effect failure cleanup of `RuntimeCore.phase` and tracking stack | `incr/cells/eager_derived_wbtest.mbt` (`push abort: reactive failure restores phase stack and sources`, `push abort: effect failure restores phase stack and sources`) uses a wbtest-only push-frame harness because current public push closures are non-raising; `incr/cells/phase_wbtest.mbt` covers normal phase enter/leave; `incr/cells/event_broadcast_hook_wbtest.mbt` (`memo_event: mutation guard rejects push tracking frame`) confirms push tracking frames are observable to guards. | **Covered by harness.** The cleanup shape restores `Idle`, clears tracking/global runtime state, and preserves the last committed source list without introducing internal `.mbti` API drift in Phase 0. |
| Batch rollback | `incr/cells/batch_wbtest.mbt` (`batch: raised error rolls back pending writes and restores depth`, `batch: nested raised error rolls back entire outer batch`, `batch: failed inner batch_result rolls back before outer continues`, `batch_result: returns Err and rolls back`); `incr/tests/traits_test.mbt` (`trait: batch via Database rolls back on raised error`); `incr/tests/scope_test.mbt` (`scope: dispose during batch — pending signal writes discarded`) | **Covered.** Local and public-facade rollback paths are pinned. |
| `Watch` / `Scope` GC survival | `incr/tests/target_facade_test.mbt` (`facade derived: watch reads outside graph and keeps target alive`, `facade reachable derived: watch reads outside graph`, `facade eager derived: watch reads outside graph`, `facade scope: add_watch disposes target watch`); `incr/tests/gc_test.mbt`; `incr/cells/gc_wbtest.mbt`; `incr/tests/scope_test.mbt`; `incr/cells/scope_test.mbt`; `incr/cells/target_facade_wbtest.mbt` (`facade scope: target handles are disposed with scope`) | **Covered.** Watch roots, GC root counts, scope disposal ordering, and watched target survival across `gc()` are pinned. |
| Datalog fixpoint no-regression smoke tests | `incr/cells/datalog_wbtest.mbt` (`fixpoint: transitive closure — edge(a,b)+edge(b,c) derives path(a,c)`, `fixpoint: semi-naive recursion preserves delta frontier across iterations`, `fixpoint: terminates when no new facts derived`, `fixpoint: relation created during rule execution converges in same call`, `fixpoint: push Reactive downstream of Relation recomputes before on_change`); `incr/cells/datalog_map_relation_wbtest.mbt`; `incr/tests/integration_test.mbt` (`cross-engine: fixpoint fires on_change only when facts change`) | **Covered.** Relation, map-relation, convergence, pull invalidation, and push/on-change interaction smoke paths are pinned. |
...
2. **Private `incr/cells/` buffer and drain hook.** Runtime-owned code receives those
   summaries, buffers them, and drains only from safe `incr/cells/` facade points.
...
fast path stays allocation-free. The `incr/cells/` facade should pass `None` unless
...
payloads must be visible from `incr/cells/`; public/root `.mbti` drift is not
accepted.
